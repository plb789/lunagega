		...
		case MP_SERVER_PWRUP:
			{
				...
				// situation : ÂµÃ°Â½ÂºÃ†Â®Â¸Â®ÂºÃ¤Ã†Â®Â°Â¡ Ã„Ã‘ÃÂ® ?Ã–Â°Ã­ Â¿Â¡?ÃŒÃÂ¯Ã†Â®Â°Â¡ ÃÂ¢Â¼Ã“Ã‡ÃŸ?Â» Â¶Â§ usercount = 0;
...
//----
	...
	// MaxUserÃÂ¦Ã‡Ã‘ÂµÂµ LoginCheckQueryÂ¾ÃˆÂ¿Â¡Â¼Â­....
	...
	SafeStrCpy( bufPN, pmsg->strSecurityPW, MAX_NAME_LENGTH+1 );
	...
	if(isNoWebLauncher)
	{
		...
		if(strcmp(bufPW, "") == 0)
		{
			//LoginError( dwConnectionIndex, pmsg->AuthKey, LOGIN_ERROR_INVALID_SEC_PASS );
			//DisconnectUser( dwConnectionIndex );
			return;
		}
	...
	{
		// LUJ£¬ÓÉWebÍÅ¶ÓÖ¸¶¨¡£·þÎñ·þÎñÆ÷£º5£¬²âÊÔ·þÎñÆ÷£º55
		const int gameKey = SERVERLISTMGR->GetBillingInfo().dwConnectionIndex;
		// ÆäÊµ´Ë´¦ÍêÈ«¿ÉÒÔÖ±½ÓÆÁ±Î
		// Êý¾Ý¿âÎÞUP_WEBLAUNCHER_LOGIN_WITH_LOGINKEY´æ´¢¹ý³Ì
...
void Userconn_Notify_UserLogIn_Ack(DWORD dwConnectionIndex, char* pMsg) 
{
	...
	//ê·¸ëŸ¬ë¯€ë¡?LoginCheckDeleteê°€ ?˜ë©´ ?ˆëœ??
