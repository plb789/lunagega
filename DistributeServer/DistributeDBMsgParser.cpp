...
void WebLauncherLoginWithLoginKey( DWORD dwConnectionIndex, char* loginKey, int nGameKey, char* ip, char* securityPw )
{
	...
	if (!loginKey) return;
	if (!ip) return;
	if (!securityPw) return;

	char tempSecurityPW[17] = { 0, };

	if (strlen(securityPw) <= 0)
	{
		SafeStrCpy(tempSecurityPW, "''", 17);
	}
	else
	{
		SafeStrCpy(tempSecurityPW, securityPw, 17);
	}

	if (IsCharInString(loginKey, "'")|| IsCharInString(ip, "'"))	return;
	if (strlen(tempSecurityPW) > strlen("''") && IsCharInString(tempSecurityPW, "'")) return;
